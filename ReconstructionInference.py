...
for RECON_DIR in RECONSTRUCTION_DIRS:
    ...
    for chunk in doc.chunks:
        ...
        prediction_shapegroup.color = (255, 100, 100, 255)
        ...
        rope_line = [shape for shape in shapes if shape.geometry.type == Metashape.Geometry.Type.LineStringType]
        ropeline_chunk = np.array([chunk_marker_dict[idx] for idx in rope_line[0].geometry.coordinates])
        ...
        for cam in tqdm(cameras[3::1][:CAM_IDX_LIMIT]):
            ...
            img_depth_np = np.frombuffer(img_depth_ms.tostring(), dtype=np.float32).reshape((img.shape[0], img.shape[1], 1))
            ...
            for mask, box, score in list(zip(masks, bboxes, scores)):
                ...
                if edge_box[0] <= scallop_centre[0] <= edge_box[2] and edge_box[1] <= scallop_centre[1] <= edge_box[3]:
                    ...
                    # filters for low confidence, weird shapes
                    if spf.is_scallop(scaled_pc_lengths):
                        ...
                        within_transect = spf.polyline_dist_thresh(scallop_center_chunk, ropeline_chunk, LINE_DIST_THRESH / chunk_scale)
                        ...
                        if within_transect:
                            ...
                            new_shape.label = "scallop_sc{}_pcaw{}_circw{}".format(round(score.item(), 2),
                                                                      round(scaled_pc_lengths[0], 3),
                                                                      round(width_scallop_circle, 3))
                            new_shape.geometry.type = Metashape.Geometry.Type.PolygonType
                            polygon = [shapes_crs.project(chunk_transform.mulp(Metashape.Vector(pnt))) for pnt in scallop_polygon_chunk.T]
