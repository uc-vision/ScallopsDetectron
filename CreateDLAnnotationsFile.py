...
from utils import scallop_poly_functions as spf
import json
...
for RECON_DIR in RECONSTRUCTION_DIRS:
    ...
    for chunk_idx, chunk in enumerate(doc.chunks):
        ...
        for cam in tqdm(chunk.cameras):
            ...
            if xyz_cov_mean < CAM_COV_THRESHOLD:
                ...
                for scallop_id, polygon in enumerate(chunk_polygons):
                    ...
                    max_xy_dist = np.max(np.linalg.norm(polygon_cam[:2], axis=0))
                    if max_xy_dist > CAM_SCALLOP_DIST_THRESH / chunk_scale:
                        continue
